...
- `local-sdk-repo/`: ローカル Maven リポジトリ。デフォルトで SDK 解決に使用

## 実行前の準備

### 1. デフォルトのローカル Maven リポジトリを用意する

まず SDK リポジトリ側で `./build_android_sdk.sh` を実行し、その後このリポジトリ直下に `local-sdk-repo/` が存在することを確認してください。
...
Maven Central 側の公開物を明示的に検証したい場合は次を使います。
...
### 2. この Demo はデフォルトでローカル Maven リポジトリ方式を使います

Central の公開待ち期間でも安定して検証できるようにするためです。Maven Central を明示的に試す場合だけ `-PusePublishedSdk=true` を指定してください。
