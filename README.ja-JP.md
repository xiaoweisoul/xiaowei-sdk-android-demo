...
本リポジトリは、Android アプリへの SDK 組み込みを確認するためのサンプルプロジェクトです。`com.xiaowei.sdk:session-core:1.0.6` をアプリに統合し、最小構成で会話セッションを接続する流れを確認できます。

このリポジトリ自体には SDK のバイナリは含まれていません。実行前に、受領した `xiaowei-sdk-android-1.0.6-maven-repo.zip` をリポジトリ直下の `local-sdk-repo/` に展開してください。

詳細情報はこちらをご参照ください: http://www.xiaoweisoul.vip/docs/app-access-overview

## この Demo で確認できること

- ローカル Maven リポジトリ方式で SDK を正しく組み込めているかの確認
...
- `local-sdk-repo/`: ローカル Maven リポジトリ。デフォルトでは Git 管理対象外

## 実行前の準備

### 1. SDK のローカル Maven リポジトリを用意する

受領した zip をリポジトリ直下に展開してください。
...
SDK zip をまだ `local-sdk-repo/` に展開していない場合、ビルドは失敗し、先にローカル Maven リポジトリを配置するよう案内されます。
...
### 2. この Demo はローカル Maven リポジトリ方式で SDK を組み込みます

目的は、ソースコードモジュール依存や社内専用パスではなく、実際の顧客導入に近い方式を示すことです。
...
### ビルド時に `com.xiaowei.sdk:session-core:1.0.6` が見つからない

次を確認してください。

- `local-sdk-repo/` が存在するか
- ディレクトリ構成が正しいか
- `com/xiaowei/sdk/session-core/1.0.6/` が実際に含まれているか
